...
public class RegexMatch {
    ...
    int isMatch(char[] word, int i, char[] pattern, int j, int[][] resultCache) {
        ...
        if (i > word.length - 1) {
            ...
            return getMatchFromCache(word, i, pattern, j + 1, resultCache);
        }
...
//        System.out.println("i= " + i + " j= " + j + " " + word[i] + " " + pattern[j]);
        ...
        if (pattern[j] == '*') {
            int matchStar = getMatchFromCache(word, i + 1, pattern, j, resultCache);
            int matchNextOfStar = getMatchFromCache(word, i, pattern, j + 1, resultCache);
        ...
        if (word[i] == pattern[j] || pattern[j] == '?') {
            return getMatchFromCache(word, i + 1, pattern, j + 1, resultCache);
        } else {
            return 0;
        }
    ...
    private int getMatchFromCache(char[] word, int i, char[] pattern, int j, int[][] resultCache) {
        if (resultCache[i][j] == -1) {
            resultCache[i][j] = isMatch(word, i, pattern, j, resultCache);
        }
